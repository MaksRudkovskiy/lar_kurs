...
@section('content')
    <div class="w-full h-full p-10">
        ...
        <form action="{{ route('language_update') }}" class="flex flex-col max-w-36" method="post">
        @csrf
        <label for="language" class="dark:text-white">{{__('profile-personalisation.choose')}}</label>
        ...
        <button type="submit" class="dark:text-white bg-custom-303134 rounded h-8 m-3">{{__('profile-personalisation.save')}}</button>

        <p class="dark:text-white">Current locale: {{ App::getLocale() }}</p>
